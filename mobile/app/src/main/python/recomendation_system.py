...
def recommend(name):
    ...
    for each in top31_indexes:
        ...
        df_new = pd.DataFrame(columns=['address_name','name','cuisines', 'general_rating', 'avg_price'])
    for each in recommend_restaurant:
        df_new = df_new.append(pd.DataFrame(df_percent[['address_name','name','cuisines', 'general_rating', 'avg_price']][df_percent.index == each].sample()))
